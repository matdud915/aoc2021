...
getAllFlashesStepNumber :: M.Map (Int, Int) Int -> Int
getAllFlashesStepNumber m = helper 0 m
    where
        helper :: Int -> M.Map (Int, Int) Int -> Int
        helper i m = if all (== 0) (M.elems m)
            then i
            else
                let
...
main = do
    ...
    let iteratedMap = getAllFlashesStepNumber indexedMap
    print iteratedMap